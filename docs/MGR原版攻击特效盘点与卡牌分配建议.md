...
- 其余当前攻击牌中，《时间线之东》本身不直接造成伤害，不应强行添加敌方命中特效。
...
- `Yaaaaaa / 呀啊啊啊`：普通钝击，保持短促直接，不使用波动或飞行斩击效果。
- `LightUp / 点亮舞台`：每段伤害在敌人脚底投下一束短促舞台追光；代码绘制且无显式等待，仍适合高频演奏触发。
...
2. 再处理《金枪鱼横斩》《流☆星☆群》《星辰导引》《立方棱镜》《百鬼夜行》等视觉语义非常明确的牌。
...
- 诅咒与凝视：夜行少女、被害妄想携带女子、百鬼夜行、注视。
- 《最后一拍》使用代码绘制的枪口闪光、瞬时弹道与命中闪光，不再归入斩击特效。
- 《时间线之东》不直接造成伤害，继续只依赖音符与乐谱反馈。
