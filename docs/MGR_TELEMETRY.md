...
- 幕数 `1..10`，地图节点总数不超过 `200`，单节点房间数不超过 `16`；
- 有效对局时长不超过 `72` 小时，进阶不超过 `100`，重载次数不超过 `1000`；
- 最终牌组 `1..500` 张，遗物不超过 `300` 个，药水不超过 `50` 瓶；
- 最大生命不超过 `10000`，最大能量和基础音符槽各不超过 `100`，金币不超过 `10000000`；
