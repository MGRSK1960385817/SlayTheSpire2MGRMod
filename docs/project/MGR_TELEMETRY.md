...
`DataAnalysis/Analyze-MgrCardMetrics.py` 使用最新 `.jsonl.gz` 快照生成 [`MGR卡牌胜率分析.md`](../../DataAnalysis/Reports/MGR卡牌胜率分析.md)、[`MGR卡牌选取率分析.md`](../../DataAnalysis/Reports/MGR卡牌选取率分析.md) 和完整 CSV 明细。报告仅包含卡牌聚合，不输出 Steam ID、安装 ID、事件 ID或单局路线。

原版会把战斗奖励、商店库存、事件、先古与部分特殊卡牌选择统一写入 `card_choices`。因此分析不得直接把全字段当作奖励选取率：

1. 仅使用 schema 7 及以上、`tracking_complete=true` 的唯一正式对局；分析层再次按 `event_id` 去重。
2. 仅把 `resolved_room_type` 为 `Monster`、`Elite` 或 `Boss` 的 `card_choices` 视为战斗卡牌奖励；排除商店、事件、先古、休息点与宝箱。
3. 卡牌目录来自当前内容登记和简中标题，只纳入启用、非联机专属、白/蓝/金稀有度的正常 MGR 卡；排除基础牌、衍生牌、先古牌和联机牌，升级版本合并到同一卡牌 ID。
4. 选取率以候选条目数为分母、`picked=true` 条目数为分子。它衡量“出现时有多常被抓”，不是最终卡组占有率。
5. 胜率主口径以“至少在战斗奖励中抓取过一次该牌的唯一对局”为样本；同一局多张同名牌只计一次。报告同时给出“本局见过但从未抓取”的对照胜率、最终卡组存在胜率、95% Wilson 区间、中位抓取层和最新遥测构建结果。
6. 两份报告同时列出最高与最低排名，并按白、蓝、金三个稀有度分别给出前五与末五。胜率排名至少需要20个抓取对局，选取率排名至少需要30次合规战斗奖励出现；高低榜使用完全相同的样本和门槛。
7. 这些结果是观察性数据，不是随机实验。稀有度、抓取层数、玩家经验、进阶、卡组适配与版本数值都会造成混杂；高胜率或低胜率只能进入平衡复查队列，不能直接证明某牌需要削弱或加强。

2026-08-20 快照从 1379 条云端原始行中按稳定 `event_id` 去除一条重复，得到 1378 条唯一事件；分析跳过一条 schema 6，并排除一条把首个先古节点误记为12点回复的 schema 7 不一致记录，最终使用 1376 局，胜利 793 局，总体胜率 57.6%。报告覆盖当前正常单人卡池 80 张牌及 69730 个合规 MGR 战斗奖励候选条目。

2026-08-27 快照分页取得3505条云端原始行，按稳定`event_id`去除11条重复后写入3494条唯一事件。分析跳过一条schema 6旧记录，并排除一条不满足守恒规则的schema 7异常记录，最终使用3492局，胜利1981局，总体胜率56.7%；当前80张正常单人卡映射完整，共统计178555个合规MGR战斗奖励候选条目。春岚在精确A0中的白卡选取率与抓取后胜率分别排名5/20和15/20，在精确A10中分别排名6/20和9/20；完整分组样本、置信区间和观察性解释见[`MGR卡池设计与分析.md`](../design/MGR卡池设计与分析.md#115-春岚的分进阶白卡表现2026-08-27)。

2026-08-29 快照分页取得3697条云端原始行，去除12条重复后写入3685条唯一事件；分析跳过一条schema 6旧记录，并排除同一条已知守恒异常记录，最终使用3683局，胜利2097局，总体胜率56.9%，共统计188796个合规MGR战斗奖励候选条目。全历史白卡最低选取率为注视2.9%，蓝卡最低为活动小丑10.1%；完整末五及最新构建趋势见[`MGR卡池设计与分析.md`](../design/MGR卡池设计与分析.md#116-白卡与蓝卡最低选取率快照2026-08-29)。

## 遗物胜率分析口径

`DataAnalysis/Analyze-MgrRelicMetrics.py`使用同一套schema、`tracking_complete`、一致性检查和`event_id`去重规则生成[`MGR遗物胜率分析.md`](../../DataAnalysis/Reports/MGR遗物胜率分析.md)与聚合CSV。主口径为“本局曾获得该遗物”的唯一对局胜率：最终遗物列表与逐层`relic_choices.picked=true`取并集，以保留后来被替换或移除的遗物；同一局同名遗物只计一次。历史`MGR_MOD_RELIC_METRONOME`会合并到当前`MGR_MOD_RELIC_CLICK_TRACK`。

同稀有度排名最低要求20个获得对局，只比较能够从当前MGR源码或本地原版源码可靠确认稀有度的遗物。玩家安装的其他模组遗物即使出现在遥测中，也不会在缺乏可信稀有度时进入排名。初始遗物和初始遗物升级版单独展示，不与随机奖励遗物排名。报告同时提供95% Wilson区间、中位获得层和A0/A10+拆分；由于遗物获取时间、路线、商店购买与幸存者偏差非常显著，排名只能用于筛选复查对象，不能直接解释为遗物的因果强度。
