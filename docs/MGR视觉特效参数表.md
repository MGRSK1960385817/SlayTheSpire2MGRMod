...
万象音符额外使用更明亮、低饱和的流动色谱，并在存在期间持续向外生成彩色星屑。`Notes.OmniaNoteSparkSeconds = 0.10` 控制星屑生成间隔，`Notes.OmniaNoteMaximumSparks = 14` 控制单颗万象音符同时保留的星屑上限；星屑只影响视觉，不进入战斗随机数与音符结算。

选择过程的全屏滤镜集中在 `Scripts/Mechanics/MgrSelectionScreenVfx.cs`：想象、创造使用真实屏幕采样黑白滤镜；失败作少女使用像素化、RGB 错位、扫描线、噪点、横向撕裂与偶发蓝屏。滤镜生命周期由选择流程持有，选择完成、取消或异常退出都会自动释放。
...
《春岚》结算后，`MgrSpringStormVfx` 将 `images/vfx/SpringStormVfx.png` 短暂铺满战斗画面并进行小幅随机抖动。现在它只作一次施放闪光，不再与 `TemporaryFortePower` 的整回合存续时间绑定。该层忽略鼠标输入，不会阻挡选牌或点击。
