...
万象音符使用更明亮、低饱和的流动色谱，并持续轮换五种基础音符与星空音符轮廓。常驻外抛星屑已经移除，避免继续维护一套在高分辨率音符贴图缩放链路中不稳定的附属粒子。

选择过程的全屏滤镜集中在 `Scripts/Mechanics/MgrSelectionScreenVfx.cs`：想象、创造使用独立高层 `CanvasLayer` 与显式全屏回拷完成真实屏幕采样、完全去色及1.38倍对比度拉伸。能力叠加时，先在一次手牌选择中选取至多能力层数张牌，再按选择顺序逐张决定类型，最后统一替换；整批操作共用一次滤镜与 `audio/Writing.ogg` 循环。失败作少女的故障层位于原版 OverlayStack 下方，二选一使用原版“发现”式居中界面，因此候选卡不会被故障覆盖。所有滤镜和音频在完成、取消或异常退出时都会自动释放。
...
所有 `Maguro*` 攻击牌都会调用 `MgrFishVfx`，令 `images/vfx/fish.png` 从角色方向掠过目标。主体图片起始透明度固定为 `0.9`，后方附带较淡的青色与紫色虚影；整个动画不等待、不改变伤害时机，因此连续演奏不会因鱼影额外变慢。
...
每当任意来源真正生成一个星空音符，`MgrStarryNoteVfx` 都会从屏幕上方落下少量代码绘制的小星星。逻辑位于统一音符生成入口，因此星降之海、孤独宇宙、遗物、能力和双倍音符均使用同一反馈，不再保留卡牌专属星雨。

| 参数 | 当前值 | 作用 |
| --- | ---: | --- |
| `MinimumStarsPerNote / MaximumStarsPerNote` | `2 / 4` | 每生成一个星空音符随机落下的星星数量 |
| `ZIndex` | `190` | 坠星在战斗特效容器内的绘制层级 |

### 春岚全屏覆盖

《春岚》结算后，`MgrSpringStormVfx` 将 `images/vfx/SpringStormVfx.png` 短暂铺满战斗画面，以10像素幅度抖动0.30秒，再用0.24秒淡出。现在它只作一次施放闪光，不再与 `TemporaryFortePower` 的整回合存续时间绑定。该层忽略鼠标输入，不会阻挡选牌或点击。

| 参数 | 当前值 | 作用 |
| --- | ---: | --- |
| `Opacity` | `0.68` | 图片主体不透明度 |
| `FlashHoldSeconds` | `0.18s` | 图片保持闪现与抖动的时间 |
| `ShakeAmplitude` | `7px` | 抖动目标在水平、垂直方向上的最大偏移 |
| `ShakeTargetSeconds` | `0.045s` | 随机更换一次抖动目标的间隔 |
| `ShakeSmoothing` | `24` | 追随新抖动目标的灵敏度 |
| `DrawPadding` | `16px` | 图片在屏幕边缘额外外扩，避免抖动时露出空隙 |
| `FadeOutSeconds` | `0.20s` | 闪现后的淡出时间 |
